...
fn main() {
    ...
    // 0の個数をc0, 1の個数をc1とすると、c0 >= 1 かつ c1 >= 1 なら絶対に1箇所は隣り合っているので、除去を行うことが可能
    // したがって終了条件は c0 == 0 or c1 == 0
    // 1回の除去で 0 と 1 が1つずつ消えるため、
    // min(c0, c1) 回除去を行うとこの終了条件が満たされる
    let c0 = c.iter().filter(|&&a| a == '0').count();
    let c1 = c.len() - c0;
